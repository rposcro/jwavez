...
import com.rposcro.jwavez.serial.probe.exceptions.FrameException;
...
import com.rposcro.jwavez.serial.probe.frame.SerialFrame;
import com.rposcro.jwavez.serial.probe.utils.ByteBuffer;
import com.rposcro.jwavez.serial.probe.utils.TimeoutKeeper;
  ...
  public void handleInboundSOF(ByteBuffer buffer) {
    ...
    try {
      if (!frameValidator.validate(buffer)) {
        ...
        transmitter.transmitData(SerialFrame.ACK_FRAME.getBuffer());
      ...
      try {
        transmitter.transmitData(SerialFrame.CAN_FRAME.getBuffer());
