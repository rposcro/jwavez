...
public class RequestCallbackFlowHelper {
  ...
  public <T extends FunctionCallback> Class<T> solicitedCallbackClass(SerialCommand command) throws FlowException {
    ...
    if (!FunctionCallback.class.isAssignableFrom(callbackClass)) {
      throw new FlowException("Correlated callback class %s is not of type %s", callbackClass, FunctionCallback.class);
    }
